function [L] = getLandMarks( specMat )
    ...
    for i = 1:numFrms
        ...
        for ii =  peakIdx  % >> check here to improve speed.
            if ( peakVec( ii ) > thresh( ii ) && pksFnd < pksPerFrame )
                ...
                gausn  = exp( -0.5 * ( ( ( 1:length(thresh) ) - ii ) / sprd ).^2 );  %Gaussian with 0 centred around 
                                                                                     %current peakIdx
    ...
    thresh = thresholdCalc( specMat( :, numFrms ) , sprd ); %% >> check thresholdCalc!! This is the last bug spot! 
    ...
    for i = numFrms:-1:1
        ...
        while ( maxIdx > 0  && maxFwd( 1, maxIdx ) == i )    % >> while condition loops for all the maxFwds that were found
            if  ( maxFwd(3, maxIdx) >= thresh( maxFwd(2, maxIdx ) ) )
                maxBck( 1, bckCnt) = i;
                maxBck( 2, bckCnt) = maxFwd(2, maxIdx);
                bckCnt  = bckCnt + 1;

                % recalculate threshold
                gausn  = exp( -0.5 * ( ( ( 1:length(thresh) ) - maxFwd( 2,maxIdx ) ) / sprd ).^2 );
                thresh = max( thresh, gausn*maxFwd(2, maxIdx) ); 
            end
            maxIdx = maxIdx - 1;
        end
        thresh = gausDec*thresh;
